 ...
 */
 ...
 ;(function($) {
	...
	var defaults = {
        ...
        // Title of header of pane
        headerTitle: "This is Admin-mode. Click this Pane to Store Messages",
	...
	Hotspot.prototype.setupWorkspace = function() {
		...
		$('<span/>', {
			html: `<p>${widget.config.headerTitle}</p>`
		}).css({
		...
		this.element.delegate('span', 'click', function(event) {
			...
			for (var i = 0; i < schema.length; i++) {
				var val = schema[i];
				var fill = prompt('Por favor ponga el ' + val.property, val.default);
				if (fill === null) {
					return;
				}
